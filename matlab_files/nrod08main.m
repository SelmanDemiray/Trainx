...
nepoch  = 1000000000;    % the number of training epochs
nbatch  = 1000;   % the number of mini matches for training
nhid    = 10000;    % the number of hidden units
sigma   = 0.1;    % the variance in the initial weights and biases

% initialize the synaptic weights and biases
W0 = randn(nhid,784); % weights from input to hidden layer
W1 = randn(10,nhid);  % weights from hidden layer to output
b0 = randn(nhid,1);   % biases at hidden layer
b1 = randn(10,1);     % biases at output layer

% initialize the weight and bias changes
delta_W0 = zeros(nhid,784); 
delta_W1 = zeros(10,nhid);  
delta_b0 = zeros(nhid,1);   
delta_b1 = zeros(10,1);     

% initialize the batch image and label holders
batch_size   = size(train_images,2)/nbatch;
batch_images = zeros(784,batch_size);
batch_labels = zeros(10,batch_size);
...
[train_r0,train_r1] = forward_pass(train_images,W0,W1,b0,b1); 
[test_r0,test_r1]   = forward_pass(test_images,W0,W1,b0,b1); 
...
for epoch = 1:nepoch
	...
	for batch = 1:nbatch
		...
		[r0,r1] = forward_pass(batch_images,W0,W1,b0,b1);

		% perform a backward pass
		[dL_by_dW0,dL_by_dW1,dL_by_db0,dL_by_db1] = backward_pass(batch_images,batch_labels,r0,r1,W0,W1,b0,b1);
